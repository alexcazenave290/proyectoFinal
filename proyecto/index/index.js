...
function crearCard(pet) {
  ...
  card.innerHTML = `
    ...
    <div class="card-body">
      ...
      <button class="adopt-button ${isAdopted ? 'adopted' : ''}" ${isAdopted ? 'disabled' : ''} data-pet-id="${pet.id}" data-creator-email="${pet.email_creador || ''}" data-creator-type="${pet.tipo_creador || ''}">
  ...
  // Agregar event listener al botón de adoptar
  const adoptButton = card.querySelector('.adopt-button');
  if (adoptButton && !isAdopted) {
...
function renderizarMensajes(mensajes) {
  ...
  mensajes.forEach(mensaje => {
    ...
    let contenidoMensaje = `<div>${escapeHtml(mensaje.contenido)}</div>`;
