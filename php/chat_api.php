...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
        // Obtener mensajes con información del emisor y mascota
        $stmt = $pdo->prepare("
            SELECT m.*, 
                ...
                masc.nom_masc as nombre_mascota,
                masc.estadoAdopt_masc as mascota_adoptada,
                COALESCE(masc.mail_empl, masc.email_inst) as creador_mascota
            FROM mensajes m
            ...
            LEFT JOIN Mascota masc ON m.id_mascota_adopcion = masc.id_masc
...
elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmtParticipantes = $pdo->prepare("
            SELECT 
                COALESCE(cp.mail_us, cp.mail_empl, cp.email_inst) as email_participante,
                COALESCE(u.nom_us, e.nomb_empl, i.nomb_inst) as nombre_participante
            FROM conversacion_participantes cp
            LEFT JOIN Usuario u ON cp.mail_us = u.mail_us
            LEFT JOIN Empleado e ON cp.mail_empl = e.mail_empl
            LEFT JOIN Institucion i ON cp.email_inst = i.email_inst
            WHERE cp.conversacion_id = ?
        ");
        $stmtParticipantes->execute([$conversacion_id]);
        $participantes = $stmtParticipantes->fetchAll(PDO::FETCH_ASSOC);
        
        // Crear notificación para el destinatario (el que no es el emisor)
        foreach ($participantes as $participante) {
            if ($participante['email_participante'] !== $email_actual) {
                $nombreEmisor = $mensaje['nombre_emisor'] ?? 'Un usuario';
                $tituloNotif = "Nuevo mensaje de $nombreEmisor";
                $contenidoNotif = substr($contenido, 0, 100) . (strlen($contenido) > 100 ? '...' : '');
                crearNotificacion($pdo, $participante['email_participante'], 'mensaje', $tituloNotif, $contenidoNotif, $conversacion_id);
            }
        }
